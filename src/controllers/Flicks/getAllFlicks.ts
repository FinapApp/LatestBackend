...
export const getAllFlicks = async (req: Request, res: Response) => {
    try {
        ...
        pipeline.push(
            ...
            // FIXED: Store original repost ID before lookup to differentiate between original and deleted reposts
            {
                $addFields: {
                    originalRepostId: '$repost'
                }
            },
            {
                $lookup: {
            ...
            },
            {
                $addFields: {
                    repost: {
                        $cond: {
                            if: { $gt: [{ $size: '$repost' }, 0] },
                            then: { $arrayElemAt: ['$repost', 0] },
                            else: {
                                $cond: {
                                    if: { $ne: ['$originalRepostId', null] },
                                    then: null, // Was a repost, but parent deleted
                                    else: '$$REMOVE' // Was never a repost, remove field entirely
                                }
                            }
                        }
                    }
            ...
            {
                $unset: 'originalRepostId' // Clean up temporary field
            },
