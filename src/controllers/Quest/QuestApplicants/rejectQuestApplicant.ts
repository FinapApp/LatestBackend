...
export const rejectQuestApplicant = async (req: Request, res: Response) => {
    try {
        ...
        const updateQuestApplicant = await QUEST_APPLICATION.findByIdAndUpdate(questApplicantId, { status: 'rejected' }, { new: true, projection: { quest: 1 , _id: 0  } });
        if (updateQuestApplicant) {
            await QUESTS.findByIdAndUpdate(updateQuestApplicant.quest, { $inc: { totalRejected: 1, leftApproved: -1 } });
