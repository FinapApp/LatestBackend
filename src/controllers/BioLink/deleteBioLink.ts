...
export const deleteBioLink = async (req: Request, res: Response) => {
    ...
    try {
        const validationError: Joi.ValidationError | undefined = validateBioLinkId(req.params, req.query);
        if (validationError) {
        ...
        if (deleteBioLink) {
            ...
            return handleResponse(res, 200, success.delete_biolink, lang)
        }
        return handleResponse(res, 404, errors.bio_link_not_deleted, lang)
    } catch (error) {
