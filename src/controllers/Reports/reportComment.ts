...
export const reportComment = async (req: Request, res: Response) => {
    ...
    try {
        const validationError: Joi.ValidationError | undefined = validateReportComment(req.body, req.params,req.query);
        if (validationError) {
        ...
        if (reportUser) {
            ...
            return handleResponse(res, 200, success.comment_reported, lang)
        }
        return handleResponse(res, 304, errors.comment_reported, lang)
    } catch (error) {
        console.log(error)
