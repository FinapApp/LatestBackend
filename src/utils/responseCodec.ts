...
const getMessage = (message: any, lang: Lang): string | null => {
  if (!message) return null;
  if (typeof message === "string") return message; // fallback
  return message[lang] ?? message["en"] ?? null;
};

/**
 * Sends a consistent response, showing only requested language for error/success messages.
 */
export const handleResponse = (
  ...
  value: any,
  lang: Lang = "en",
  meta: Array<{ message: string | null }> = []
) => {
  if (res.headersSent) return;

  // Prefer meta message if present, otherwise use value.message
  const selectedMessage = getMessage(meta?.[0]?.message, lang) || getMessage(value?.message, lang);

  if (statusCode > 205) {
    // Error response: only success:false, message, (optional: details)
    return res.status(statusCode).json({
      success: false,
      message: selectedMessage,
      // Optionally add error details (like Joi validation details) if you pass them. Remove if you don't want.
      details: meta.length ? meta : undefined,
    });
  } else {
    // Success response: only success:true and localized message or data
    if (value?.message) {
      return res.status(statusCode).json({
        success: true,
        message: selectedMessage,
